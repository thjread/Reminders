...
fn serialize_todo(t: &Todo, f: &mut fmt::Formatter) -> fmt::Result {
    write!(f, "[id]:{},\n[title]:\"{}\",\n[deadline]:{:?},\n[done]:{},\n[done_time]:{:?},\n[create_time]:{:?},\n[hide_until_done]:{},\n[highlight]:{},\n", t.id, t.title, t.deadline.map(|d| d.and_utc().timestamp()), t.done, t.done_time.map(|d| d.and_utc().timestamp()), t.create_time.and_utc().timestamp(), t.hide_until_done, t.highlight)// escape newlines in title?
}
